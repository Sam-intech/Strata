  ...
  return (
    ...
    >
      ...
      <div className="space-y-6">
        {/* Patient ID */}
        <div className="w-max flex justify-between gap-4 sm:flex-row">
          <Field label="Patient ID (optional)">
            <input type="text" className="input" placeholder="e.g. 12345" disabled={isLoading}/>
          </Field>
          <Field label="Time of assessment (optional)">
            <input type="text" className="input" placeholder="e.g. 2023-01-01" disabled={isLoading}/>
          </Field>
        </div>
        ...
        {/* Labs Section */}
        <details className="rounded-xl border border-zinc-200 p-4" open={labsOpen} onToggle={(e) => setLabsOpen((e.target as HTMLDetailsElement).open)}>
      ...
      <div className="space-y-5 flex flex-col items-center">
        ...
        <p className="w-[70%] text-center text-xs text-zinc-500">
          Disclaimer: This is a Research prototype and should not be used for final clinical decision but for decision support only.
        </p>
      </div>
    </form>
  );
